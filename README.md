...
Everything is plain Markdown. No scripts, no dependencies, no network calls, no telemetry.
...
Restart or refresh Codex to use it.
...
claude/                       Claude Code version
  ...
  skills/subagent-orchestration/
    SKILL.md
    references/               delegation contract, handoff template
codex/                        Codex version
...
Both versions share the same orchestration semantics — role contract, execution modes, acceptance gates, reuse gate, handoff rules. They differ only in platform primitives: Claude Code delegates through the `Agent` tool, reuses through `SendMessage`, and isolates writes with `isolation: "worktree"`; Codex uses `spawn_agent`, `send_input`, `wait_agent`, and `close_agent`. Each tree is self-contained, so a Claude Code session never reads Codex tool names and vice versa.
...
- A compact ledger format that survives context compaction without re-reading.
